...
public class SecurityConfigurer {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        // 拷贝一份，避免污染 authIgnoreConfig（AuthenticationTokenFilter 用它判断是否跳过 token 校验）
        List<String> permitAll = new ArrayList<>(authIgnoreConfig.getIgnoreUrls());
        // SSE 流式端点单独加入 permitAll：async dispatch 时 AuthorizationFilter 会重跑，
        // 但响应式线程上 SecurityContext 已丢失，直接放行避免 AuthorizationDeniedException。
        // Token 校验仍由 AuthenticationTokenFilter 在前置 Filter 层完成（不受此影响）。
        permitAll.add("/ai/chat/send/stream");
        permitAll.add("/error");
        permitAll.add("/v3/**");
