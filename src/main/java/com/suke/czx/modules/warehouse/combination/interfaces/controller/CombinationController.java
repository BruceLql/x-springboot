...
public class CombinationController extends AbstractController {
    ...
    public R list(@RequestParam(required = false) String skuName,
                  @RequestParam(required = false) Integer status,
                  @RequestParam(defaultValue = "1") Long page,
                  @RequestParam(defaultValue = "10") Long size) {
        IPage<SkuDefinition> result = combinationApplicationService.queryPage(skuName, status, page, size);
    ...
    public R info(@PathVariable String skuId) {
        SkuDefinition sku = combinationApplicationService.getDetail(skuId);
    ...
    public R create(@RequestBody CreateSkuRequest request) {
        String skuId = combinationApplicationService.createSku(
                request.getSkuName(),
                request.getSkuImage(),
                request.getPreAllocateQuantity(),
                request.getGoodsIds()
        );
    ...
    public R availableGoods() {
        List<Goods> list = combinationApplicationService.getAvailableGoods();
